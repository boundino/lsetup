                     ...
                     T branchvalue)
{
  ...
  std::cout<<"*** This file will be changed: "<<std::endl<<"  "<<inputname<<std::endl;
  ...
  T brval;
  if(tr->FindBranch(branchname.c_str()))
    { std::cout<<__FUNCTION__<<": error: branch "<<branchname<<" exists. "<<std::endl; return; }
  ...
  if(argc == 6 || argc == 5)
    {
      ...
      std::string argvoutput = xjjc::str_replaceall(argvinput, ".root", 
                                                    Form("__add_%s_%s.root", xjjc::str_replaceallspecial(argv[2]).c_str(), xjjc::str_replaceallspecial(argv[3]).c_str()));
      ...
      if(thistype=="Int_t" || thistype=="int" || thistype=="Bool_t" || thistype=="bool")
        {
          ...
          xjjc::addbranch(argvoutput, argv[2], argv[3], val_int); return 0;
        }
      if(thistype=="Float_t" || thistype=="float")
        {
          ...
          xjjc::addbranch(argvoutput, argv[2], argv[3], val_float); return 0;
        }
      if(thistype=="Double_t" || thistype=="double")
        {
          if(!xjjc::str_isnumber(thisvalue)) { std::cout<<__FUNCTION__<<": inconsistent value and type: ("<<thistype<<")"<<thisvalue<<std::endl; return 2; }
          xjjc::addbranch(argvoutput, argv[2], argv[3], val_double); return 0;
        }
      if(thistype=="TString")
        {
          xjjc::addbranch(argvoutput, argv[2], argv[3], val_string); return 0;
        }
      else 
        {
